...
namespace ng
{
  namespace graphics
  {
    GLShader::GLShader(std::string vertexFilename, std::string fragmentFilename) :
      _vertexFilename(vertexFilename), _fragmentFilename(fragmentFilename)
    {
      _vertexID = 0;
      _fragmentID = 0;
      _programID = 0;
    ...
    {
      if (!GLShader::_compileShader(_vertexID, GL_VERTEX_SHADER, _vertexFilename))
        return (false);
      if (!GLShader::_compileShader(_fragmentID, GL_FRAGMENT_SHADER, _fragmentFilename))
        return (false);
      if (!GLShader::_compileProgram(_programID, _vertexID, _fragmentID))
        return (false);
      return (true);
    }
    ...
    void GLShader::_getCompileLog(GLuint obj)
    {
      ...
      {
        LOG(ERROR) << "GLShader compilation failure: unknown object type" << std::endl;
        return;
      }
      msg = new char[size + 1];
      if (glIsShader(obj) == GL_TRUE)
    ...
    bool GLShader::_compileShader(GLuint &shader, GLenum type, std::string const &filename)
    {
      ng::io::File file;
      ng::io::Buffer buff;
      GLchar *source;
      ...
      {
        LOG(ERROR) << "GLShader compilation failure: cannot open file: " << e.what() << std::endl;
        glDeleteShader(shader);
        return (false);
      }
      source = (GLchar *) (buff.getPtr());
    ...
    bool GLShader::_compileProgram(GLuint &program, GLuint vertex, GLuint fragment)
    {
      GLint status;
